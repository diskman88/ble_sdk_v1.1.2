...
static void is_unpair_old_device(dev_addr_t *event, void *data)
{
    static dev_addr_t retain_addr;

    LOGI(TAG,"paired %x,%x,%x,%x,%x,%x\r\n",event->val[0],event->val[1],event->val[2],event->val[3],event->val[4],event->val[5]);
    pairnum ++;
    if(memcmp(event, data, sizeof(dev_addr_t))) {
        retain_addr = *event;
    }
    if (pairnum >= CONFIG_BT_MAX_PAIRED) {
        ble_stack_dev_unpair(&retain_addr);
        ble_stack_white_list_remove(&retain_addr);
        LOGI(TAG,"unpair old %x,%x,%x,%x,%x,%x\r\n",retain_addr.val[0],retain_addr.val[1],retain_addr.val[2],retain_addr.val[3],retain_addr.val[4],retain_addr.val[5]);
    }
}

static void is_unpair_max_device(dev_addr_t *event, void *data)
{
    LOGI(TAG,"paired %x,%x,%x,%x,%x,%x\r\n",event->val[0],event->val[1],event->val[2],event->val[3],event->val[4],event->val[5]);
    pairnum ++;
    if(pairnum >= CONFIG_BT_MAX_PAIRED) {
        ble_stack_dev_unpair(event);
        ble_stack_white_list_remove(event);
        LOGI(TAG,"unpair old %x,%x,%x,%x,%x,%x\r\n",event->val[0],event->val[1],event->val[2],event->val[3],event->val[4],event->val[5]);
    }
}
...
static void event_smp_complete(ble_event_en event, void *event_data)
{
    ...
    if (e->err == 0) {
        ...
        pairnum = 0;
        int ret = ble_stack_paired_dev_foreach(is_unpair_old_device,(dev_addr_t *)(&e->peer_addr));
        if (ret != 0) {
            LOGI(TAG, "parameter err\r\n");
        }
    } else {
        ret = ble_stack_disconnect(e->conn_handle);
        if(ret) {
            LOGE(TAG,"disconnect fail");
        }
    }
...
static void smp_key_miss(ble_event_en event, void *event_data)
{
    LOGI(TAG, "Please unbound the device");
}
...
static int event_callback(ble_event_en event, void *event_data)
{
    ...
    case EVENT_SMP_KEY_MISS:
        smp_key_miss(event, event_data);
        break;
...
aos_sem_t g_wait_first_trigger;

#define KEY_ROW_NUM 4
#define KEY_COLUMN_NUM 4
key_io key_row[KEY_ROW_NUM] = {{P14},{P15},{P18},{P20}};
key_io key_column[KEY_COLUMN_NUM] = {{P23},{P25},{P31},{P32}};
key_map map_set[KEY_ROW_NUM * KEY_COLUMN_NUM] = {{0x59},{0x5A},{0X5B},{0x48},{0X5C},{0x5D},{0x5E},{0X4A},{0x5F},{0x60},{0x61},{0x39},{0X28},{0X27},{0X29},{0x66}};

keyboard g_key = {
    .row_num = KEY_ROW_NUM,
    .column_num = KEY_COLUMN_NUM,
    .key_row = key_row,
...
void key_scan_cb(key_scan_event_e event, uint8_t len)
{
    ...
    case DATA_RECV: {
        uint8_t key_data;
        int ret = 0;
        ret = keyboard_get_ranks_data(&key_data,1);
        if(ret > 0) {
            send_data.Code6 = key_data;
            ret = key_board_send(&send_data);
            if(ret) {
                LOGE(TAG,"Send Keyboard data faild\r\n");
            } else {
                LOGD(TAG,"send data %02x",key_data);
            }
        } else {
            LOGE(TAG,"Read Keyboard data faild\r\n");
        }
...
int app_main(int argc, char *argv[])
{
    ...
    keyboard_init(&g_key,key_scan_cb);

    cli_reg_cmd_keysend();

    aos_sem_new(&sync_sem, 1);
    aos_sem_new(&g_wait_first_trigger,1);
    ...
    pairnum = 0;
    int ret = ble_stack_paired_dev_foreach(is_unpair_max_device, &pairnum);
    if (ret != 0) {
        LOGI(TAG, "parameter err\r\n");
    }
    ...
    while (1) {
        aos_sem_wait(&g_wait_first_trigger, AOS_WAIT_FOREVER);
        if(g_conn_handle != -1) {
            keyboard_start_scan();
        }
    }
